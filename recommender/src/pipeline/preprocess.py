...
def unify_and_format_domain(df: pd.DataFrame, domain: str) -> pd.DataFrame:
    ...
    if domain == "steam":
        ...
        adult_tags   = df['tags'].astype(str).str.contains(r'NSFW|Nudity|Sexual Content|Hentai|Adult', case=False, na=False)
        ...
        # Filter out itch.io entries — overwhelmingly amateur/abandoned games
        ...
        adult_tags  = df['tags'].astype(str).str.contains(r'NSFW|Nudity|Sexual Content|Adult', case=False, na=False)
        mature_esrb = _safe_col(df, 'esrb_rating', '').astype(str).str.contains(r'Adults Only|Mature', case=False, na=False)
