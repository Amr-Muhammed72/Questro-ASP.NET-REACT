...
const LandingPage = () => {
  ...
  useEffect(() => {
    if (!isLoading && trendingMedia && trendingMedia.length >= 3 && displayMedia.length === 0) {
      const movies = trendingMedia.filter(m => m.type === 'movie');
      const games = trendingMedia.filter(m => m.type === 'game');
      
      const shuffle = (arr) => [...arr].sort(() => 0.5 - Math.random());
      
      const combined = shuffle([...movies, ...games]);
      
      if (combined.length < 3) {
         setDisplayMedia(trendingMedia);
      } else {
  ...
  const isDataLoading = isLoading || displayMedia.length < 3;

  const item1 = displayMedia[currentIndex % displayMedia.length];
  const item2 = displayMedia[(currentIndex + 1) % displayMedia.length];
  const item3 = displayMedia[(currentIndex + 2) % displayMedia.length];
          ...
          <div className="flex-1 relative hidden lg:flex items-center justify-center h-[600px] w-full max-w-lg" style={{ perspective: "2000px" }}>
            ...
            >
              ...
              <motion.div
                ...
                className="absolute bottom-[5%] left-[-5%] w-[200px] h-[300px] rounded-2xl overflow-hidden shadow-[0_20px_50px_rgba(0,0,0,0.5)] border border-white/10 z-10 glassmorphism transform -rotate-12 bg-zinc-900"
              >
                ...
                {isDataLoading ? (
                  ...
                  <AnimatePresence mode="popLayout">
                    {item1 && (
                      <motion.img 
                        key={item1.imageUrl}
                        initial={{ opacity: 0, filter: "blur(4px)", scale: 1.1 }} animate={{ opacity: 1, filter: "blur(0px)", scale: 1 }} exit={{ opacity: 0, filter: "blur(4px)", scale: 0.9 }} transition={{ duration: 1.5 }}
                        src={item1.imageUrl} alt={item1.title} className="absolute inset-0 w-full h-full object-cover" 
                      />
                    )}
                  </AnimatePresence>
                )}
              ...
              <motion.div
                ...
                className="absolute top-[0%] right-[-5%] w-[220px] h-[320px] rounded-2xl overflow-hidden shadow-[0_20px_50px_rgba(0,0,0,0.5)] border border-white/10 z-20 glassmorphism transform rotate-6 bg-zinc-900"
              >
                <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent z-10" />
                {isDataLoading ? (
                  <div className="absolute inset-0 w-full h-full bg-zinc-800 animate-pulse" />
                  ...
                  <AnimatePresence mode="popLayout">
                    {item2 && (
                      <motion.img 
                        key={item2.imageUrl}
                        initial={{ opacity: 0, filter: "blur(4px)", scale: 1.1 }} animate={{ opacity: 1, filter: "blur(0px)", scale: 1 }} exit={{ opacity: 0, filter: "blur(4px)", scale: 0.9 }} transition={{ duration: 1.5 }}
                        src={item2.imageUrl} alt={item2.title} className="absolute inset-0 w-full h-full object-cover" 
                      />
                    )}
                  </AnimatePresence>
                )}
              ...
              <motion.div
                ...
                className="relative z-30 w-[300px] h-[450px] rounded-2xl overflow-hidden shadow-[0_30px_60px_rgba(0,0,0,0.7)] border border-white/20 glassmorphism bg-zinc-900"
              >
                  ...
                  <AnimatePresence mode="popLayout">
                    {item3 && (
                      <motion.img 
                        key={item3.imageUrl}
                        initial={{ opacity: 0, filter: "blur(4px)", scale: 1.1 }} animate={{ opacity: 1, filter: "blur(0px)", scale: 1 }} exit={{ opacity: 0, filter: "blur(4px)", scale: 0.9 }} transition={{ duration: 1.5 }}
                        src={item3.imageUrl} alt={item3.title} className="absolute inset-0 w-full h-full object-cover" 
                      />
                    )}
                  </AnimatePresence>
                )}
