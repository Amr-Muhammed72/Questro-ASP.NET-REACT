...
const MovieCard = memo(({ movie, progress, isRowItem = false }) => {
  ...
  return (
    <div className="flex flex-col group/card cursor-pointer items-center w-full">
      <div  
      onClick={handleClick}
      className={`relative flex-shrink-0 transition-transform duration-300 ease-out hover:scale-105 hover:z-30 rounded-md overflow-hidden bg-zinc-900 ${containerWidthClass} aspect-[2/3]`}
      >
        {displayImage ? (
