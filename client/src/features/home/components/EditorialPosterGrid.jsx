...
import { Star, Gamepad2, Film, ChevronLeft, ChevronRight } from 'lucide-react';
...
const EditorialPosterGrid = ({ 
  ...
  const carouselItems = (items || []).slice(0, 10).map(item => {
    ...
    if (isGame && rawRating) {
      ...
      finalRating = item.rating ? item.rating : item.originalData?.tmdbRating;
    }
    ...
    return {
      ...
      linkId: item.originalData?.rawgId || item.originalData?.tmdbId || item.id?.replace('movie-', '')?.replace('game-', ''),
      rating: finalRating,
    ...
    return (
      <section className="relative w-full py-12 overflow-hidden group/section">
        <div className="max-w-7xl mx-auto px-6">
          {/* Skeleton Header */}
          <div className="mb-6">
            <div className="w-32 h-4 bg-white/5 rounded animate-pulse mb-3" />
            <div className="w-56 h-8 bg-white/10 rounded-lg animate-pulse" />
          </div>
  ...
  return (
    <section className="relative w-full py-12 overflow-hidden group/section">
      <div className="max-w-7xl mx-auto px-6">
        ...
        {/* HEADER */}
        <div className="mb-6">
          <p className="text-sm font-bold text-indigo-400 uppercase tracking-widest mb-1.5">
            {label}
          </p>
                ...
                <Link to={`/${item.type === 'game' ? 'games' : 'movies'}/${item.linkId}`} className="block w-full">
                  <div className="relative aspect-[16/9] rounded-2xl overflow-hidden shadow-lg mb-4 bg-zinc-900">
                    ...
                    <img 
                      ...
                      aria-hidden="true"
                    />
                    {/* Foreground Image - Object Contain prevents cropping */}
                    <img 
                      src={item.imageUrl} 
                      alt={item.title} 
