    ...
    <div className="space-y-6">
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {/* Max Movie Rating */}
          <div>
            <h4 className="text-sm font-semibold text-zinc-300 mb-3 uppercase tracking-wider">Max Movie Rating</h4>
            <div className="flex flex-wrap gap-2">
              <button
                type="button"
                onClick={() => onChangeMaxContentRating(null)}
              ...
              {movieRatingCaps.map((rating) => (
                <button
                  key={rating}
                  type="button"
                  onClick={() => onChangeMaxContentRating(rating)}
                  className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-all border ${
          ...
          {/* Max Game Rating (Metacritic 1-5 scale mapped) */}
          <div>
            <h4 className="text-sm font-semibold text-zinc-300 mb-3 uppercase tracking-wider">Max Game Rating</h4>
            <div className="flex flex-wrap gap-2">
              <button
                type="button"
                onClick={() => onChangeMaxMetacriticRating(null)}
                className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-all border ${
                  !maxMetacriticRating
                    ? 'bg-indigo-600/30 border-indigo-500 text-white'
                    : 'bg-zinc-900/60 border-zinc-700/50 text-zinc-400'
              ...
              {gameRatingCaps.map((rating) => (
                <button
                  key={rating.value}
                  type="button"
                  onClick={() => onChangeMaxMetacriticRating(rating.value)}
                  className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-all border ${
                    maxMetacriticRating === rating.value
                      ? 'bg-purple-600/30 border-purple-500 text-white shadow-[0_0_10px_rgba(138,43,226,0.3)]'
                      : 'bg-zinc-900/60 border-zinc-700/50 text-zinc-400 hover:border-zinc-500 hover:text-zinc-200'
                  }`}
                >
